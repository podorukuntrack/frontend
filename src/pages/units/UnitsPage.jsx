        ...
        <form onSubmit={handleAdd} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select className="input" value={addForm.project_id} required
            ...
            <div className="space-y-1.5">
              ...
              <select className="input" value={addForm.cluster_id} required disabled={!addForm.project_id}
                onChange={(e) => setAddForm({ ...addForm, cluster_id: e.target.value, jumlah_unit: 1, nomor_units: [""] })}>
                <option value="">-- Pilih Cluster --</option>
                {filteredClusters.map((c) => <option key={c.id} value={c.id}>{c.nama_cluster}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-4">
            <div>
              ...
              <input type="number" min="1" max={addForm.cluster_id ? sisaKuota : 50} className="input w-full sm:w-32" value={addForm.jumlah_unit} onChange={handleJumlahChange} disabled={!addForm.cluster_id || sisaKuota === 0} required />
